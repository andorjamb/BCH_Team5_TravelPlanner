    ...
    <div className="welcome-user">
      ...
      <div className="user-info">
        ...
        <div className="user-hello">
          ...
          {user?.displayName && (<div><h3 className="black-lower"> Hello, {setName(user?.displayName)}</h3>
            <button className="logout" onClick={handleSignOut}>Logout</button></div>)}

          {!user?.displayName && (<div><span>Welcome Guest</span><br />
            <Link to='/profile/signin'>Sign in</Link></div>)}
